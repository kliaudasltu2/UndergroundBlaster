...
public class BronzeCoin extends  Item {
    ...
    public BronzeCoin(PlayScreen screen, float x, float y) {
        super(screen, x, y);

        TextureAtlas atlas = screen.assets.manager.get("sprites/item/bronze.pack", TextureAtlas.class);

        //TextureRegion frame = new TextureRegion(atlas.findRegion("gold", 30));
        Array<TextureRegion> frames = new Array<>();
        for(int i = 21; i <= 30; i++){
            frames.add(new TextureRegion(atlas.findRegion("Bronze",i)));
        }
